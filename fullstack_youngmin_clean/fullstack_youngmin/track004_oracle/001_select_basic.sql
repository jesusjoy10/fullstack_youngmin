...
show user;  -- 실행) 현재 줄 선택하고 ctrl + enter 
...
-- Q8. EMP 테이블의 job, deptno 열의 중복을 제거하지 않고 그대로 모두 조회하시오
 select all job, deptno from emp;

-- Q9. EMP 테이블의 열에서 연산식을 이용하여 '연간 총수입'을 조회하시오
 select ename,sal, sal812+comm, comm from emp;

-- Q10. EMP 테이블의 열열 더하기
 select ename, sal , sal+sal+sal+sal+sal+sal+sal+sal+sal+sal+sal+comm,comm from emp;

-- Q11. EMP 테이블의 열 별칭을 사용하여 '연간총수입'을 조회하시오
 select ename,sal, sal812+comm, as annsal , comm from emp;

-- Q12. emp 테이블의 모든 열을 급여기준으로 오름차순 정렬하시오
 select * from emp order by sal;

-- Q13. emp 테이블의 모든 열을 급여기준으로 내림차순 순으로 정렬하시오.
...
--emp 테이블 값넣기 2. 조작어(DML) - intsert(삽입) , select(조회), update(수정), delete(지우기)
...
-- Ex002. 
select empno as employee_no, ename as employee_name, job, mgr as manager, hiredate, sal as salary,
comm as commission, deptno as department_no from emp order by deptno desc, ename asc;
