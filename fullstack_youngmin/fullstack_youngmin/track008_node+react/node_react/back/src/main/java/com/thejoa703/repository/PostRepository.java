...
public interface PostRepository extends JpaRepository<Post, Long>{ //Entity, pk
	...
	// 전체게시글 조회 - Oracle 네이티브 페이징
	 ...
	 List<Post>findPostWithPaiging(@Param("start")int start, @Param("end")int end);
	 ...
	 // 내가 쓴 글 + 내가 리트윗한 글 (합쳐서 조회)
	    ...
	    List<Post> findMyPostAndRetweetsWithPaging(@Param("userId") Long userId ,@Param("start")int start, @Param("end")int end);
