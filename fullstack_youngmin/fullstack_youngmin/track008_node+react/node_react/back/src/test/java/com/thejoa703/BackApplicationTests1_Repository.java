...
class BackApplicationTests1_Repository {
	...
	//---------------------------------------------------------------
	// ImageRepository (이미지생성, 단건조회, 삭제 후 조회 불가확인
	//---------------------------------------------------------------
	@Test
	...
	void testImageRepository() {
		...
		// 삭제 후 조회 불가확인 (delete:객체, findById:번호)
	...
	void testPostRepository() {
		...
		// 전체글 페이징
		List<Post> pagedPosts = postRepository.findPostWithPaiging(1, 3);
