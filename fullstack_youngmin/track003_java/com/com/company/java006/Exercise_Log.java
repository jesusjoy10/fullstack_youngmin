...
public static void main(String[] args) {
    ...
    // 목표 자동 설정 (요구사항 반영)
    goalWeight = weight - 5;    // 체중 -5kg
    goalMuscle = muscle + 5;    // 근육 +5kg
    goalFat = fat - 5;          // 체지방 -5%
    ...
    for (;;) {
        ...
        if (sel == 9) {
            ...
            if (Math.abs(muscle - goalMuscle) <= tolerance &&
                Math.abs(fat - goalFat) <= tolerance &&
                Math.abs(weight - goalWeight) <= tolerance) {
