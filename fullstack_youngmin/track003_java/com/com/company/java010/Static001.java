...
class Farm{
	//상태 - 멤버변수
	String name; // 인스턴스변수- heap area -new o - 생성자/ cat (1번지).name , cat(2번지).name
	int age;  // 인스턴스변수-heap area -new o - 생성자 / dog(2번지).name , dog(2번지).age
	
	static String FarmName="(주) 동물농장"; // 클래스.변수- Farm.FarmName
...
public class Static001 {
	public static void main(String[] args) {
        ...
        cat.name="boy"; cat.age=3; cat.show();
        Farm dog = new Farm(); 
...
[method: 정보, static, final : 공용정보]  
  Farm.class , Static001.class                   ★ 클래스는 : 설계도 
  FarmNum=2; FarmBoss="신동엽";  FarmName="(주) 동물농장";
------------------------------------
[heap: 동적]            | [stack : 잠깐빌리기]
2번지{name,age}     <- dog[1번지]  
1번지{name,age}     <- cat[1번지]
                     | main
------------------------------------
*/
