       ...
       response.setContentType("text/html; charset=UTF-8");	    
		...
		if(path.equals("/list.do")) {
		   // ■  MbtiList   -데이터 받고 처리
			...
			request.getRequestDispatcher("mbitboard/list.jsp").forward(request, response);
			
		} else if (path.equals("/writeView.do")) {
			request.getRequestDispatcher("mbitboard/write.jsp").forward(request, response);	
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			}else {out.println("<script>alert('관리자에게 문의바랍니다'); location.href='list.do'; </script>");
			}
////////////////////////////////////////////////////////////////////////////////////////////////////////////		
		} else if (path.equals("/detail.do")) {
			// ■  MbtiDetail       /          mbtiBoard/detail.jsp 
			service = new MbtiDetail(); service.exec(request, response);
			request.getRequestDispatcher("mbitboard/detail.jsp").forward(request, response);
			
		} else if (path.equals("/editView.do")) {
			// ■  MbtiUpdateView   /          mbtiBoard/edit.jsp 
			service = new MbtiUpdateView(); service.exec(request, response);
			request.getRequestDispatcher("mbitboard/edit.jsp").forward(request, response);	
			...
			request.getRequestDispatcher("mbitboard/delete.jsp").forward(request, response);	
