...
/* 클래스의 재사용
  ...
  MobileNote9     { battery / newShow()}
    ---------------------------------------------
  MobileNote7 my9 = new MobileNote9();
  ---------------------------------------------
  1) MobileNote9 는 MobileNoted 이다.
  2) 생성자호출 :  MobileNote9() →MobileNote8() →MobileNote7() → MobileNote() → Object
  3) 객체생성 : Object → MobileNote → MobileNote7 →MobileNote8()  →  MobileNote9()
  
   3번지 :  { battery / newShow()} → { face / newShow()} →    {iris, face / newShow()} →  {show()}
    ---------------------------------------------
  MobileNote7 my8 = new MobileNote8();
  ---------------------------------------------
  1) MobileNote8 는 MobileNoted 이다.
  2) 생성자호출 :  MobileNote8() →MobileNote7() → MobileNote() → Object
  3) 객체생성 :Object → MobileNote → MobileNote7 →MobileNote8()
  ...
  3) 객체생성 : Object → MobileNote → MobileNote7
   ...
   1번지 : {face/ newShow() } → {iris, face / newShow()} →  {show()}
   
   오버라이딩 : 상속시 부모와 같은 메서드 자식에게 맞게 수정 
*  */
