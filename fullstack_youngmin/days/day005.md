...
- Q3. git[로컬]에서 day005.md 클라우드와 수정한 다른부분 수정 올리기
- Q4. Q3 발생하는 오류 수정
```bash
git push origin master
To https://github.com/jesusjoy10/fullstack_youngmin.git
 ! [rejected]        master -> master (fetch first)
error: failed to push some refs to 'https://github.com/jesusjoy10/fullstack_youngmin.git'
...
```
1. 발생문제 :Git에서 원격 저장소(GitHub)에 이미 변경된 내용이 있는데, 로컬 저장소가 그 변경을 반영하지 않은 상태에서 push를 시도
2. 해결방안 : 원격 변경 사항을 먼저 가져온 후 push
```
