...
public class MethodEx010 { 
	public static void who_are_you(String[][] pets) {
		Scanner scanner = new Scanner(System.in);
		String name = "";
		String result = "등록된 동물 이름이 없습니다.";
		System.out.print("동물 이름을 입력해주세요 > ");
		name = scanner.next();
		for (int i = 0; i < pets.length; i++) {
			if (name.equals(pets[i][0])) {
				result = pets[i][0] + "는 " + pets[i][2] + " 예약이 되어 있어요.";
				break;
			}
		}
	...
	public static void who_change(String[][] pets) {
		Scanner scanner = new Scanner(System.in);
		String name = "";
		String number = "";
		...
		for (int i = 0; i < pets.length; i++) {
			if (pets[i][0].equals(name) && pets[i][1].equals(number)) {
				System.out.println("변경하실 진료 항목을 입력해주세요");
				String jinryo = scanner.next();
				pets[i][2] = jinryo;
