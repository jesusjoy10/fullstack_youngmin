...
public class ForEx007_1 {
	public static void main(String[] args) {
		//변수
		double avg=0.0;
		int total=0; int kor=-1; int math=-1; int eng=-1; 
		String std; 
		...
		for(;;) { //1, 무한반복
			if(! (0<=kor && kor <=100)) { //2-1 점검 : 0~100 사이가 아니라면
			System.out.println("국어점수 입력>");kor= scanner.nextInt(); //2-2 입력받고
				continue; // 2-3 아래건너뛰기 - 2-1 으로 가서 범위맞는지 확인
		}
		    if(! (0<=math && math <=100)) {
